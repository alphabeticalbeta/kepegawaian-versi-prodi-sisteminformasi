...
<div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden mb-6">
    ...
    <div class="p-6">
        <div class="grid grid-cols-1 gap-6">
            <div>
                <label class="block text-sm font-semibold text-gray-800">Dokumen Pendukung Jabatan Fungsional Tertentu</label>
                <p class="text-xs text-gray-600 mb-2">Dokumen pendukung untuk jabatan fungsional tertentu (1 File)</p>
                @if($isViewOnly)
                    @if(isset($usulan->data_usulan['dokumen_usulan']['dokumen_pendukung_jabatan_fungsional_tertentu']['path']))
                        <div class="space-y-3">
                            ...
                            <div class="flex items-center gap-3 p-3 bg-white border border-gray-200 rounded-lg">
                                ...
                                <div class="flex-1">
                                    <div class="text-sm font-medium text-gray-800">Dokumen Pendukung Jabatan Fungsional Tertentu</div>
                                    <div class="text-xs text-gray-500">{{ basename($usulan->data_usulan['dokumen_usulan']['dokumen_pendukung_jabatan_fungsional_tertentu']['path']) }}</div>
                                </div>
                                <a href="{{ asset('storage/' . $usulan->data_usulan['dokumen_usulan']['dokumen_pendukung_jabatan_fungsional_tertentu']['path']) }}" 
                                   target="_blank" 
                                ...
                                <a href="{{ asset('storage/' . $usulan->data_usulan['dokumen_usulan']['dokumen_pendukung_jabatan_fungsional_tertentu']['path']) }}" 
                                   download="{{ basename($usulan->data_usulan['dokumen_usulan']['dokumen_pendukung_jabatan_fungsional_tertentu']['path']) }}"
                    ...
                    @endif
                @else
                    <input type="file" name="dokumen_pendukung_jabatan_fungsional_tertentu" accept=".pdf" required
                           class="block w-full border-gray-300 rounded-lg shadow-sm bg-white px-4 py-3 text-gray-800 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 @error('dokumen_pendukung_jabatan_fungsional_tertentu') border-red-500 focus:ring-red-500 focus:border-red-500 @enderror">
                    <p class="mt-1 text-xs text-gray-500">Format: PDF (Max: 1MB) - Wajib</p>
                    @error('dokumen_pendukung_jabatan_fungsional_tertentu')
